...
function s.rmcon(e, tp, eg, ep, ev, re, r, rp)
    return eg:IsExists(Card.IsControler, 1, nil, 1 - tp) -- Kiểm tra nếu có lá bài được gửi xuống mộ của đối thủ.
end

-- Mục tiêu: Loại bỏ các lá bài đó
function s.rmtg(e, tp, eg, ep, ev, re, r, rp, chk)
    if chk == 0 then return eg:IsExists(Card.IsAbleToRemove, 1, nil) end -- Kiểm tra nếu có lá bài có thể bị loại bỏ.
    Duel.SetTargetCard(eg) -- Đặt mục tiêu là các lá bài vừa được gửi xuống mộ.
    Duel.SetOperationInfo(0, CATEGORY_REMOVE, eg, #eg, 0, 0)
end

-- Hành động: Loại bỏ các lá bài đó
function s.rmop(e, tp, eg, ep, ev, re, r, rp)
    local g = eg:Filter(Card.IsRelateToEffect, nil, e) -- Lọc các lá bài liên quan đến hiệu ứng.
    if #g > 0 then
        Duel.Remove(g, POS_FACEUP, REASON_EFFECT) -- Loại bỏ các lá bài đó.
    end
end
